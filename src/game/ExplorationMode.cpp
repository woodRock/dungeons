...
ExplorationMode::ExplorationMode(Registry* registry, GLRenderer* renderer, DungeonEditor* editor)
    : m_Registry(registry), m_Renderer(renderer), m_Editor(editor) {}

void ExplorationMode::Init(Camera* camera, Entity& playerEntity) {
    m_Editor->Init(m_Registry, m_Renderer);
    ...
    m_Registry->AddComponent<Transform3DComponent>(playerEntity, {0.0f, -5.0f, 2.0f, M_PI * 0.5f, -0.2f});
    m_Registry->AddComponent<PlayerControlComponent>(playerEntity, {10.0f, 0.003f, 6.0f});
    m_Registry->AddComponent<PhysicsComponent>(playerEntity, {0, 0, 0, 15.0f, true, true, 0.0f, 5.0f}); 
    m_Registry->AddComponent<WeaponComponent>(playerEntity, {0.0f, 0.0f, false});

    if (camera) {
        camera->x = 0.0f; camera->y = -5.0f; camera->z = 2.0f;
        camera->yaw = M_PI * 0.5f; camera->pitch = -0.2f;
    }
}
