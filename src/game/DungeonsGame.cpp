...
void DungeonsGame::OnUpdate(float deltaTime) {
  ...
  if (m_State == GameState::MainMenu) {
    ...
    if (!inventoryOpen) {
      ...
      // Escape to Pause Menu when inventory is closed
      if (Input::IsKeyPressed(SDL_SCANCODE_ESCAPE)) {
          ...
          m_MenuSelection = 0;
              ...
              // Editor Specific Logic (Handles its own inventory input)
...
void DungeonsGame::PlaySpatialSfx(Mix_Chunk *chunk, float x, float y,
                                   float z) {
