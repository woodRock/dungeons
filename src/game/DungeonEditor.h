...
enum class ActionResult { None, Placed, Broken, InteractedDoor, InteractedChest };
...
public:
    ...
    void Init(Registry* registry, GLRenderer* renderer);
    ActionResult Update(float dt, const Camera& cam);
    void RenderUI(GLRenderer* renderer, TextRenderer* textRenderer, int width, int height);
    void Render3D(GLRenderer* renderer);
...
private:
    void ScanAssets();
    ActionResult HandleInput(const Camera& cam);
    void UpdateCursor(const Camera& cam);
    ActionResult ExecuteAction(bool place);
    void DrawHotbar(GLRenderer* renderer, TextRenderer* textRenderer, int w, int h);
    void DrawInventory(GLRenderer* renderer, TextRenderer* textRenderer, int w, int h);
