...
DungeonsGame::DungeonsGame() : Application("Dungeons", 1280, 720) {}
...
void DungeonsGame::OnStart() {
  ...
  if (!m_GLRenderer.LoadTexture(
          "dungeon", "assets/dungeons/Assets/textures/dungeon_texture.png")) {
    std::cerr << "Failed to load dungeon texture" << std::endl;
  }

  // Load floor mesh
  if (!m_GLRenderer.LoadMesh(
          "floor", "assets/dungeons/Assets/obj/floor_tile_large.obj")) {
