...
bool GLTFLoader::Load(const std::string& path, Mesh& outMesh) {
    ...
    if (data->skins_count > 0) {
        ...
        if (skin->inverse_bind_matrices) {
            for (size_t i = 0; i < skin->joints_count; i++) {
                cgltf_accessor_read_float(skin->inverse_bind_matrices, i, &ibms[i * 16], 16);
            }
        } else {
            for(size_t i=0; i<skin->joints_count; i++) {
                Mat4 id; memcpy(&ibms[i*16], id.m, 64);
            }
        }
        ...
        for (cgltf_size i = 0; i < skin->joints_count; ++i) {
            ...
            cgltf_node_transform_local(jointNode, outMesh.skeleton.bones[i].localMatrix);
        }

        for (cgltf_size i = 0; i < skin->joints_count; ++i) {
            cgltf_node* jointNode = skin->joints[i];
            if (jointNode->parent && nodeToBoneIdx.count(jointNode->parent)) {
                outMesh.skeleton.bones[i].parentIndex = nodeToBoneIdx[jointNode->parent];
            } else {
        ...
        for (cgltf_size pIdx = 0; pIdx < mesh->primitives_count; ++pIdx) {
            ...
            for (cgltf_size aIdx = 0; aIdx < prim->attributes_count; ++aIdx) {
                if (prim->attributes[aIdx].type == cgltf_attribute_type_position) {
                    posAcc = prim->attributes[aIdx].data; break;
                }
            }
            if (!posAcc) continue;
            ...
            for (cgltf_size aIdx = 0; aIdx < prim->attributes_count; ++aIdx) {
                ...
                for (cgltf_size k = 0; k < attr->data->count; ++k) {
                    ...
                    if (attr->type == cgltf_attribute_type_position) cgltf_accessor_read_float(attr->data, k, &v.x, 3);
                    else if (attr->type == cgltf_attribute_type_normal) cgltf_accessor_read_float(attr->data, k, &v.nx, 3);
                    else if (attr->type == cgltf_attribute_type_texcoord) cgltf_accessor_read_float(attr->data, k, &v.u, 2);
                    else if (attr->type == cgltf_attribute_type_joints) {
                        uint32_t j[4]; cgltf_accessor_read_uint(attr->data, k, j, 4);
                        v.joints[0]=(int)j[0]; v.joints[1]=(int)j[1]; v.joints[2]=(int)j[2]; v.joints[3]=(int)j[3];
                    }
                    else if (attr->type == cgltf_attribute_type_weights) cgltf_accessor_read_float(attr->data, k, v.weights, 4);
                }
            }
    ...
    for (cgltf_size i = 0; i < data->animations_count; ++i) {
        ...
        for (cgltf_size j = 0; j < anim->channels_count; ++j) {
            ...
            if (!chan->target_node || !chan->target_node->name) continue;
            AnimationChannel sChan;
            sChan.boneName = chan->target_node->name;
            if (chan->target_path == cgltf_animation_path_type_translation) sChan.path = AnimationChannel::Translation;
            else if (chan->target_path == cgltf_animation_path_type_rotation) sChan.path = AnimationChannel::Rotation;
            else if (chan->target_path == cgltf_animation_path_type_scale) sChan.path = AnimationChannel::Scale;
            else continue;
            for (cgltf_size k = 0; k < chan->sampler->input->count; ++k) {
                ...
                if (kf.time > sAnim.duration) sAnim.duration = kf.time;
            }
...
std::vector<SkeletalAnimation> GLTFLoader::LoadAnimations(const std::string& path) {
    Mesh tempMesh;
    if (Load(path, tempMesh)) return tempMesh.animations;
    return {};
}
