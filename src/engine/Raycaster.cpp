...
void Raycaster::Render(SDL_Renderer *ren, const Camera &cam, const Map &map,
  ...
  for (int x = 0; x < w; x++) {
    ...
    // Mapping is complex, simplified: just draw a fixed sun in skybox texture
    // logic Or just a bright gradient at top Let's stick to gradient for now,
    // maybe add a circle later if needed. Horizon glow (White)
  ...
  for (int y = h / 2 + (int)cam.pitch + 1; y < h; y++) {
    ...
    for (int x = 0; x < w; x++) {
      ...
      if (tile == 4) {
        ...
        // Abyss Shadow (Darker as it goes down? Actually uniform darkness
        // implies depth here)
